...
MedFlux::MedFlux(QWidget *parent)
...
void MedFlux::setupTables()
{
    ...
    // Update Pos Data when position 1 and 9 are inputted

    connect(ui->positionOne, SIGNAL(valueChanged(double)), this, SLOT(pos_one(double)));
    connect(ui->positionNine, SIGNAL(valueChanged(double)), this, SLOT(pos_nine(double)));

    ui->tablePosData->show();
}

void MedFlux::setupActions()
{
...
void MedFlux::setupRecentSettings()
{
...
//void MedFlux::setupSoftKeyboard()
//{
...
void MedFlux::closeEvent(QCloseEvent *e)
{
    cleanup();
    e->accept();
}

void MedFlux::loadFile()
{
...
void MedFlux::saveFile(const QString &name)
{
    model.edited = false;
    QFile file(name);
...
void MedFlux::saveSettingsFile()
{
    ...
    settings.setValue("Position_Nine", ui->positionNine->text());
...
void MedFlux::loadSettingsFile(int num)
{
    QSettings app_settings (QSettings::IniFormat, QSettings::UserScope, "MunnTech", "Flow Cell Control");
    ...
    ui->positionNine->setValue(settings.value("Position_Nine", "").toDouble());
...
void MedFlux::about()
{
    QMessageBox::about(this, tr("About Edit"),tr("MedFlux Flow Cell Editor 2.0 \nA Qt application.\n""(c) 2014-2017 MunnTech, Open Source Material"));
}

void MedFlux::cleanup()
{
//    softKeyboard->cleanup();
}

void MedFlux::pos_one(double in)
{
    for(int i = 0; i < 12; ++i)
...
void MedFlux::pos_nine(double in)
{
    for(int i = 12; i < 24; ++i)
